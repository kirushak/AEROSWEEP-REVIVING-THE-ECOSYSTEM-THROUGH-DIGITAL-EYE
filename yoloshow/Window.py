...
class YOLOSHOWWindow(YOLOSHOW):
    ...
    def center(self):
        ...
        # Calculate center position while ensuring window fits on screen
        window_width = min(size.width(), screen.width() - 100)  # Leave some margin
        window_height = min(size.height(), screen.height() - 100)
        
        # Center the window
        self.move((screen.width() - window_width) / 2,
                 (screen.height() - window_height) / 2 - 10)

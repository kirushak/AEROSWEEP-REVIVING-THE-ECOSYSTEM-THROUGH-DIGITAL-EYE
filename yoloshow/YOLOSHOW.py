...
class YOLOSHOW(QMainWindow, YOLOSHOWBASE):
    ...
    def changeModel(self):
        ...
        # Store current model in environment for trash detection logic
        full_model_path = f'{self.current_workpath}/ptfiles/{self.model_name}'
        os.environ["CURRENT_MODEL_PATH"] = full_model_path
        
        # Check if this is a trash detection model
        model_name_lower = self.model_name.lower()
        is_trash_model = "trash" in model_name_lower or model_name_lower == "yolov10.pt" or model_name_lower == "mobilenet_trash_detector.pt"
        
        # Set appropriate confidence threshold
        if is_trash_model:
            conf_threshold = 0.15  # Lower threshold for trash detection
            self.ui.conf_spinbox.setValue(conf_threshold)
            self.ui.conf_slider.setValue(int(conf_threshold * 100))
            self.showStatus(f"Setting lower confidence threshold ({conf_threshold}) for trash detection model")
        ...
        if yolo_thread is not None:
            ...
            # Update confidence threshold in the thread
            if is_trash_model:
                yolo_thread.conf_thres = 0.15
        else:
            self.yolo_threads.set(model_name, MODEL_THREAD_CLASSES[model_name]())
            self.initModel(yoloname=model_name)
            self.loadConfig()

...
class YOLOSHOWVS(QMainWindow, YOLOSHOWBASE):
    ...
    def initModel(self, yoloname=None):
        ...
        if yoloname.endswith("left"):
            model_path = f'{self.current_workpath}/ptfiles/' + self.ui.model_box1.currentText()
        elif yoloname.endswith("right"):
            model_path = f'{self.current_workpath}/ptfiles/' + self.ui.model_box2.currentText()
            
        if not model_path or not os.path.exists(model_path):
            self.showStatus(f"Error: Model file not found - {model_path}")
            return
            
        try:
            yolo_thread.new_model_name = model_path
            ...
            self.showStatus(f"Error initializing model: {str(e)}")
            return
    ...
    def changeModelProcess(self, yoloname, mode=None):
        if mode == "left":
            ...
            if yolo_thread is not None:
                ...
                self.loadConfig()
            now_model_name = str(self.model_name1).replace(".pt", "")
            self.showStatus(f"Change Model to {now_model_name} Successfully for Left Part")
            # 重载 common 和 yolo 模块
            ...
            if yolo_thread is not None:
                ...
                self.initModel(yoloname=yoloname)
                self.loadConfig()
            now_model_name = str(self.model_name2).replace(".pt", "")
            self.showStatus(f"Change Model to {now_model_name} Successfully for Right Part")
            # 重载 common 和 yolo 模块
